...
def parse_with_groq(text: str, old_sum: int) -> int | None:
    ...
    try:
        prompt = (
            f"Tu es un assistant qui aide à compter des personnes. "
            f"Le nombre actuel est {old_sum}. "
            f"L'utilisateur envoie ce message : \"{text}\"\n\n"
            f"Ce message peut être en malgache, français, ou un simple chiffre. "
            f"Détermine le nouveau nombre total selon le message :\n"
            f"- Si c'est un nombre absolu (ex: '5', 'misy 5', 'izahay 5'), retourne ce nombre.\n"
            f"- Si c'est une addition (ex: 'miampy 2', '+2', 'tonga 2 hafa', 'nanampy 2'), retourne {old_sum} + ce nombre.\n"
            f"- Si c'est une soustraction (ex: 'mihena 1', 'lasa 1', 'nandao 1', '-1'), retourne {old_sum} - ce nombre (minimum 0).\n"
            f"- Si le message ne parle pas de nombre de personnes, retourne NULL.\n\n"
            f"Réponds UNIQUEMENT avec un entier ou NULL, rien d'autre."
        )
...
def extract_number(text: str, old_sum: int = 0) -> int | None:
    ...
    if any(w in text_clean for w in ["miampy", "nanampy", "fanampiny", "plus", "tonga"]):
        return old_sum + n
    if any(w in text_clean for w in ["mihena", "lasa", "nandao", "moins"]):
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "• /video — Admin : atombohy ny salon vidéo\n"
...
async def cmd_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("📹 Salon vidéo démarré ✅")
    except Exception as e:
