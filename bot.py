...
MINIAPP_URL  = os.environ.get("MINIAPP_URL", "https://VOTRE_DOMAINE/miniapp.html")
...
def miniapp_button():
    """Bouton inline avec lien URL simple."""
    if not MINIAPP_URL or "VOTRE_DOMAINE" in MINIAPP_URL:
        return None
...
async def cmd_debut(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await job_start_session(context)
        await update.message.reply_text("✅ Fivoriana atombohy!")
    except Exception as e:
...
async def cmd_fin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await job_end_session(context)
        await update.message.reply_text("✅ Fivoriana tapitra!")
    except Exception as e:
