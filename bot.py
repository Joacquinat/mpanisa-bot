...
async def _check_admin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """Vérifie si l'utilisateur est admin du groupe cible, peu importe d'où il écrit."""
    try:
        member = await context.bot.get_chat_member(GROUP_ID, update.effective_user.id)
        if member.status not in ("administrator", "creator"):
            await _send(context, "🚫 Admin ihany no afaka manao izany.")
            return False
        return True
    except Exception as e:
...
async def cmd_total(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not session["participants"]:
        await _send(context, "📭 _Tsy nisy isa nandefa hatreto._")
        return
    total        = session["total"]
    participants = session["participants"]
    text = (
...
def main():
    ...
    app.add_handler(CommandHandler("start",      cmd_help,      filters=group_filter))
