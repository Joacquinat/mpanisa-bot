...
async def cmd_modifier(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        args = context.args
        if len(args) < 2:
            await update.message.reply_text("❌ Fampiasana : /modifier nom 5", parse_mode="Markdown")
            return

        new_number = int(args[-1])
        search_name = " ".join(args[:-1]).lstrip("@").lower()

        data    = load_data()
        session = get_session(data)

        if not session["active"]:
            await update.message.reply_text("❌ Tsy misy session mavitrika.")
            return

        # Find participant by name
        found_id = None
        for uid, v in session["participants"].items():
            if search_name in v["name"].lower():
                found_id = uid
                break

        if not found_id:
            await update.message.reply_text(f"❌ Tsy hita : *{search_name}*", parse_mode="Markdown")
            return
        ...
        await update.message.reply_text("❌ Ny isa dia tsy mety.")
    except Exception as e:
        await update.message.reply_text(f"❌ Nisy olana: {e}")
...
async def cmd_supprimer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if not context.args:
            await update.message.reply_text("❌ Fampiasana : /supprimer nom", parse_mode="Markdown")
            return

        search_name = " ".join(context.args).lstrip("@").lower()
