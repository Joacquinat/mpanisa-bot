...
def get_wt_url() -> str:
    """URL principale des gazety jw.org."""
    return "https://www.jw.org/mg/zavatra-misy/gazety/"

def miniapp_button():
    """Bouton inline : MWB lun-ven, Tilikambo sam-dim."""
    now = datetime.now(TIMEZONE)
    dow = now.weekday()  # 0=Lundi, 5=Samedi, 6=Dimanche
    if dow >= 5:
        url  = get_wt_url()
...
async def cmd_testmwb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await _check_admin(update):
        return
    url = get_mwb_url()
    await update.message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton(text="📋 Jereo ny fandaharam-pivoriana", url=url)
        ]])
    )

async def cmd_testwt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await _check_admin(update):
        return
    url = get_wt_url()
    await update.message.reply_text(
        ...
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton(text="📖 Jereo ny Tilikambo Fiambenana", url=url)
        ]])
    )
...
def main():
    ...
    app.add_handler(CommandHandler("testmwb",    cmd_testmwb))
    app.add_handler(CommandHandler("testwt",     cmd_testwt))
